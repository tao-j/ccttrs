...
async fn load_profile(sd_path: String) -> Result<SdCardProfile, String> {
    tauri::async_runtime::spawn_blocking(move || {
        SdCardProfile::load_from_sd(Path::new(&sd_path))
...
async fn init_profile(
    ...
    tauri::async_runtime::spawn_blocking(move || {
        let profile = SdCardProfile::new(profile_type, volume_name, staging_dir, last_file_path, last_file_timestamp);
...
#[tauri::command]
async fn start_copy(app: AppHandle, state: State<'_, AppState>, sd_path: String, staging_dir: Option<String>) -> Result<(), String> {
    ...
    tauri::async_runtime::spawn(async move {
        ...
        let res = tauri::async_runtime::spawn_blocking(move || {
            ...
            crate::copy::run_sync(&sd_root, &mut profile, cancel_token, move |progress| {
                let _ = app_clone.emit("copy-progress", ProgressPayload {
